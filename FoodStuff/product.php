...
if(isset($_POST["add_to_cart"]))
{
		      ...
		      $duplicate=mysqli_query($conn,"SELECT * FROM cart WHERE p_id='$p_id' AND c_id='$c_id'");
		         if(mysqli_num_rows($duplicate)==0){
		           $insert = mysqli_query($conn, "INSERT INTO cart (c_id,p_id,v_email,qty) values ('$c_id','$p_id','$v_email','$qty')");
					      }
          ...
          <div class="row">
              <div class="col-4 col-lg-2 col-md-2  mt-4">
                ...
                <div class="list-group shadow" style="padding:0px">
                      <a  href="product.php" class="list-group-item list-group-item-action active text-white">ประเภทสินค้า</a>

                      <!--this php code will dynamically add the category from the DB-->
                      <?php

                          $cat = mysqli_query($conn, "SELECT * FROM category");//stores all categories in $cat variable
              ...
              <?php
                $e= mysqli_num_rows(mysqli_query($conn, " SELECT * from product where p_status='Available'"));
              ?>
            ...
            <div class="col-8 col-lg-10 col-md-10">
              <div class="row">
                ...
                /*if the user will search by name of product then this if will be executed and filterred data will be shown*/
                  if(isset($_GET['find'])){
                      $search = $_GET['search'];
                      $calling = mysqli_query($conn, "SELECT * FROM product WHERE p_name LIKE '%$search%' AND p_status='Available'");
                    }

                /*if the user will search category wise then this elseif will be executed and filterred data will be shown*/
                  elseif(isset($_GET['cat'])){
                      $cat = $_GET['cat'];
                      $calling = mysqli_query($conn, "SELECT * FROM product WHERE cat_title='$cat' AND p_status='Available' ");
                    }

              /* if the user will directly go to the products page then this else will be executed without filteration*/
                  else{
                      $calling = mysqli_query($conn, "SELECT * FROM product WHERE p_status='Available'");  /*collects all data from product table*/
                    }
														...
														<h6 class="text-dark">₹ <?php echo $row["p_price"]; ?>/-</h6>
															...
															<?php
																if(isset($_SESSION['customer'])):
																	...
																	$cust = mysqli_query($conn, "SELECT * from customer where c_email='$log'");
