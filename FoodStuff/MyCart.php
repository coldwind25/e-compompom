...
$d= date('Y-m-d h:i:s', time());
        ...
        if(isset($_SESSION['customer'])):
          ...
          $calling = mysqli_query($conn,"select * from customer where c_email='$log'");
	            ...
	            <div class="row" style="border: solid 1px #ccc">
	                  ...
	                  <div class="col-lg-6">
	                   ...
	                   <p>weight: <?= $cart_row['each_qty']?> </p>
	                    ...
	                    <p>Price: <?= $cart_row['p_price']?>*<?= $cart_row['qty'];?> = ₹ <?php //echo $cart_row['p_price'];?>/-</p>
	                  </div>
	                   ...
	                   <br>
															...
															//  echo $cart_row['p_price'];

												 $total = $total + ($cart_row['p_price']*$cart_row['qty']);
										 ?>
	                  </div>
	              </div>
	          </div>
...
$id=date('Ymdhis');
        ...
        foreach ($array as $key => $value){
					$insert = mysqli_query($conn, "INSERT INTO orders (invoice_no,c_id,p_id,v_email,qty) values ('$id','$value[0]','$value[1]','$value[2]','$value[4]')");
				}
				$del=mysqli_query($conn, "DELETE FROM cart where cart.c_id=".$data['c_id']);
